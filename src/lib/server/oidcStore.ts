 ...
 * DATA_DIR/oidc. The refresh token never reaches the browser: the id_token is
 * Nebula's StarRocks credential and lives ~5 minutes, while a session lives
 * 24h — holding the refresh token server-side lets any code path mint a fresh
 * id_token on demand.
 *
 * Filesystem + single process by design (adapter-node, one container). Swap
 * this module for a Redis/Postgres store before scaling out.
 */
...
function decrypt(blob: string): string | null {
	try {
		...
		// Wrong SESSION_SECRET or corrupt file — treat as absent, which forces a
 ...
 * spent. Null when the user has no stored refresh token (never signed in,
 * or signed out) or the refresh was rejected — callers should treat that as
 * "needs to sign in again" rather than a server error.
 */
...
/** The stored id_token as-is (possibly expired) — only useful as a logout hint. */
export async function storedIdToken(username: string): Promise<string | null> {
	const stored = await read(username);
