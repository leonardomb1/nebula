...
/**
 * OIDC against any standard provider (authentik is the reference). Everything
 * except the issuer is discovered, so a provider change is one env var.
 *
 * The id_token is not only proof of identity — it is also the credential
 * Nebula presents to StarRocks, whose FE is configured (by the StarRocks team,
 * never by this app) to accept the IdP's JWTs. authentik's tokens live ~5
 * minutes while a session lives 24h, so the refresh token is held server-side
 * (see ./oidcStore) and a fresh id_token is minted per warehouse connection.
 *
 * The id_token signature is NOT verified here: it arrives over the TLS back
 * channel straight from the token endpoint (OIDC Core 3.1.3.7). StarRocks
 * verifies it properly against the JWKS before honouring it.
 */
...
export async function authorizeUrl(
	...
	// Signing out of Nebula does not end the IdP session (other apps stay
