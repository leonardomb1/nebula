...
<div
	...
	role="menu"
	tabindex="-1"
	class="fixed z-50 min-w-44 rounded-md border border-line bg-hover py-1 shadow-xl shadow-black/40"
	style="left: {left}px; top: {top}px"
>
	{#each items as item (item.label)}
		<button
			role="menuitem"
			class="flex w-full items-center gap-2 px-3 py-1.5 text-left text-[13px] text-ink-muted hover:bg-active hover:text-ink"
			onclick={() => {
				item.action();
				onclose();
			}}
		>
			<Icon name={item.icon} size={13} class="text-ink-faint" />
			{item.label}
		</button>
	{/each}
</div>
