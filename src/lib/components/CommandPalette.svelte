...
>
	<div
		class="flex h-fit max-h-[60vh] w-full max-w-xl flex-col overflow-hidden rounded-lg border border-edge bg-surface-2 shadow-2xl shadow-black/50"
	>
		<div class="flex items-center gap-2 border-b border-edge px-3">
			<Icon name="search" size={15} class="text-ink-dim" />
			...
			<input
				...
				class="h-11 min-w-0 flex-1 bg-transparent text-[13px] text-ink placeholder:text-ink-dim focus:outline-none"
			/>
			{#if loading}
				<Icon name="spinner" size={14} class="animate-spin text-ink-dim" />
			{/if}
			<kbd class="rounded border border-edge px-1.5 py-0.5 text-[10px] text-ink-dim">Esc</kbd>
		</div>
		...
		<ul class="overflow-auto py-1">
			{#each hits as hit, i (hit.section + hit.hint + hit.label)}
				...
				{#if first}
					<li
						class="px-3 pt-2 pb-1 text-[10px] font-semibold tracking-widest text-ink-dim uppercase"
					>
						{hit.section}
					</li>
				{/if}
				<li>
					<button
						...
						onclick={() => choose(hit)}
					>
						<Icon name={hit.icon} size={14} class="text-ink-dim" />
						<span class="truncate">{hit.label}</span>
						{#if hit.hint}
							<span class="ml-auto truncate pl-3 font-mono text-[11px] text-ink-dim">
								{hit.hint}
							</span>
						{/if}
					</button>
			...
			{#if term.trim() && hits.length === 0 && !loading}
				<li class="px-3 py-6 text-center text-[12px] text-ink-dim">{m.search_empty()}</li>
			{/if}
		</ul>
	</div>
</div>
