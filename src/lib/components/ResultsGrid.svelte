<script lang="ts">
	...
	/** Grid density is a user preference — see the appearance settings. */
	let ROW_HEIGHT = $derived(settings.rowHeight);
	...
	function edges(r: number, c: number): string {
		...
		const sides: string[] = [];
		if (!selected(r - 1, c)) sides.push('inset 0 1px 0 0 var(--nb-accent)');
		if (!selected(r + 1, c)) sides.push('inset 0 -1px 0 0 var(--nb-accent)');
		if (!selected(r, c - 1)) sides.push('inset 1px 0 0 0 var(--nb-accent)');
		if (!selected(r, c + 1)) sides.push('inset -1px 0 0 0 var(--nb-accent)');
...
>
	...
	>
		...
		<thead class="sticky top-0 z-20">
			<tr>
				<th role="columnheader" class="sticky left-0 z-30 h-8 border-r border-b border-line bg-hover p-0">
					<button
						class="flex h-full w-full items-center justify-center text-ink-faint hover:text-ink"
						onclick={selectAll}
				...
				{#each resultset.columns as column, c (c)}
					...
					<th
						role="columnheader"
						class="relative h-8 border-r border-b border-line bg-hover p-0 font-medium
							{box && box.c0 <= c && c <= box.c1 ? 'text-accent' : ''}"
					>
						<button
						...
						>
							{#if type === 'temporal'}
								<Icon name="calendar" size={12} class="text-ink-faint" />
							{:else}
								<span class="font-mono text-[10px] leading-none tracking-tight text-ink-faint">
									{type === 'number' ? '123' : 'ABC'}
								</span>
							{/if}
						...
						<span
							...
							class="absolute top-0 right-0 h-full w-1 cursor-col-resize hover:bg-accent-hover"
							onpointerdown={(e) => startResize(e, c)}
						></span>
					</th>
				{/each}
				<th role="columnheader" class="border-b border-line bg-hover"></th>
			</tr>
		</thead>
		...
		<tbody>
			...
			{#each slice as row, i (first + i)}
				...
				<tr class="group" style="height: {ROW_HEIGHT}px">
					<td class="sticky left-0 z-10 border-r border-b border-line-soft p-0 text-right">
						<button
							class="h-full w-full bg-glass px-2 text-right text-[11px] group-hover:bg-hover
								{box && box.r0 <= r && r <= box.r1 ? 'text-accent' : 'text-ink-faint'}"
					...
					{#each resultset.columns as column, c (c)}
						...
						<td
							...
							class="overflow-hidden border-r border-b border-line-soft px-2 text-ellipsis whitespace-nowrap
								{selected(r, c) ? 'bg-select' : 'group-hover:bg-hover'}
								{row[c] === null ? 'text-ink-faint italic' : ''}
								{kind(column.type) === 'number' ? 'font-mono tabular-nums' : ''}"
						>
							{value}
						</td>
					{/each}
					<td class="border-b border-line-soft group-hover:bg-hover"></td>
				</tr>
			{/each}
	...
	{#if copied}
		<span
			class="pointer-events-none absolute bottom-3 left-3 z-30 rounded-md border border-line bg-hover px-2 py-1 text-[11px] text-ink shadow-lg"
		>
			{m.copied()}
		</span>
	{/if}
